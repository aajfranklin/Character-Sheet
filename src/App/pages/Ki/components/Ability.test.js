...
function setUp() {
  wrapper = shallow(<Ability
    ...
    deleteThisAbility={mockDeleteAbility}
    editAbility={mockEditAbility}
    save={mockSaveAbility}
    update={mockUpdateAbility}
    updateKiAvailable={mockUpdateStat}
    uuid={state.ki.abilities[0].uuid}
  ...
  describe('on rendering an ability with editing false', () => {
    ...
    describe('when the roll button is clicked', () => {
      it('should call action creator to update ki available', () => {
        ...
        expect(mockUpdateStat.mock.calls[0]).toMatchObject([2]);
      });
    });
